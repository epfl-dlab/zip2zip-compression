...
#[cfg(feature = "pytorch")]
#[pyclass(module = "zip2zip_compression")]
pub struct PyTorchCodebookManager {
    pub codebook: Codebook,
}

#[cfg(feature = "pytorch")]
#[pymethods]
impl PyTorchCodebookManager {
    #[new]
    pub fn new(config: CompressionConfig) -> Self {
        Self {
            states: Vec::new(),
    ...
    /// The `py` is the marker holding the GIL.
    ///
    /// The `ids` is the Tensor of ids to update the codebooks with.
    ///
    /// Returns a tuple containing the updates and the indices of the updates.
    pub fn update_codebooks(&mut self, ids: PyTensor) -> (Vec<Vec<usize>>, Vec<Vec<usize>>) {
        let cpu_ids = ids.to_device(Device::Cpu);
        let batch_size = cpu_ids.size()[0];
